...
module.exports = new Event("messageCreate", async (bot, message, guild) => {
 if(message.author.bot) return;
    ...
    if(req.length < 1) {
      ...
      return message.reply("<:Elexyr22:754441336849170543> Le bot a bien **enregistrez** votre serveur, vous pouvez profiter du bot ! Prefix : `e!` <a:on:973521009925836830> ")
    }
...
const time = Date.now();
        ...
        if(commandFile.permission !== "Aucune" && message.author.id !== "998210063506354236" && !message.member.permissions.has(new Discord.Permissions(commandFile.permission))) return message.reply("<:Elexyr22:754441336849170543> Vous n'avez pas la __permission requise__ pour exécuter **cette commande !** <a:Nop1:768183643020853298>").then(async mess => setTimeout(async () => {mess.delete()}, 5000))
